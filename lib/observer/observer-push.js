...
var _hasObserver = async (req, res) =>{
    ...
    _config.listeners.forEach(el => {
        ...
        if (urlObs == req.path && el.method == req.method){
            obs = {id:el.id, destination: el.destination, uri:el.uri, method: el.method, statuscode: el.statuscode}; 
        }
    });
...
var _nofity = async (req, res, body, obs) => {
    ...
    let url = "htp://localhost:8200/add/flow/observer/:metadataId/push"
    _log("_nofity obs.uri",obs)
