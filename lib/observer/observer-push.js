...
exports.factory = (options) => {
    ...
    _log("factory publish", _config.urlPublish);
    _loadObservers();
    _log("factory publish 2", _config.urlPublish);
    return middleware;
}
...
var _loadObservers = async () => {
    ...
    _log("_loadObservers _pckJson.name", _pckJson.name);
    _log("_loadObservers publish", _config.urlPublish);
    _log("_loadObservers url", url);
