...
var _getMetadataId = async (req, res, body) =>{
    ...
    console.log("METADATA metaInfo", body.metaInfo)
    if(body && body.metaInfo && body.metaInfo.metadataId){
        metaId = body.metaInfo.metadataId;
    }else if(body && body.metadata && body.metadata.id){
        metaId = body.metadata.id;
    }else if(body && body.metadataId){
        metaId = body.metadataId;
    }
    //TODO - pesquisar na request, header, etc
    
    console.log("METADATA metaId", metaId)
    return metaId
}
