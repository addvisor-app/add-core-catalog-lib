...
var _log = async (...args) => {
    if(_config.debug){
        ...
        console.log("[",date, "ADD CATALOG LIB - OBSERVER",pckJson.name,"]", ...args);
    }
}

var _error = async (...args) => {
    let [date] = new Date().toLocaleString('pt-BR').split(', ');
    console.error("[",date, "ADD CATALOG LIB - OBSERVER ERROR",pckJson.name,"]", ...args)
}