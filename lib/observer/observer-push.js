...
var _setConfig = (options) =>{
    ...
    const urlPublish = url + "'/add/flow/observer/publish?app=" + _pckJson.name
    var response = await axios.get(urlPublish).catch(err =>{_error(err.config.url, "ERROR:", err.message)});;
