...
module.exports = class AddAxiosFoundry extends AddAxios{
    ...
    async _getHost(){
        ...
        if(!this._api.baseURL){
            const urlRegistry = "https://ad-core-flow-api-"+this._subaccount+"."+this._domain+"/add/flow/catalog/binding/"+ this._destination
            console.log("######## CATALOG SCF urlRegistry", urlRegistry);
            const binding = await this._api.get(urlRegistry);
            if(binding){
                console.log("######## CATALOG SCF binding", binding);
                this._api.baseURL = binding.protocol+"://"+binding.host;
            }else{
                //Workarround
