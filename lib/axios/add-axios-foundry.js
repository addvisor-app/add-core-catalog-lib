...
module.exports = class AddAxiosFoundry extends AddAxios{
    ...
    async _getHost(){
        ...
        if(!this._api.baseURL){
            this._log('AddAxiosFoundry ADD_GLOBAL_VARIABLE_URL', process.env.ADD_GLOBAL_VARIABLE_URL);

            let urlRegistry  = process.env.ADD_GLOBAL_VARIABLE_URL.replace("/add/flow/runtime/variables", "/add/flow/catalog/binding")
