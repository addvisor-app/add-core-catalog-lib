...
var _getCatalog = async (app) =>{
    ...
    const resp = await axios.get( process.env.ADD_GLOBAL_VARIABLE_URL +'/ADD_DOMAIN', _header).catch(err => { 
        _error("Erro para buscar DOMAIN: ", pckJson.name,"error:",err.message)
    });
    const domain = (resp && resp.data) ? resp.data.value : null;

    const resp2 = await axios.get( process.env.ADD_GLOBAL_VARIABLE_URL +'/ADD_SUBACCOUNT', _header).catch(err => { 
        _error("Erro para buscar SUBACCOUNT: ", pckJson.name,"error:",err.message)
    });
