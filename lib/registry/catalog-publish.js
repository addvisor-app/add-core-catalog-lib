...
var _getCatalog = async (app) =>{
    ...
    const host = "https://add-core-flow-api-addvisor-dev.cfapps.us10.hana.ondemand.com/add/flow/runtime/variables";

    const resp = await axios.get(host+'/ADD_DOMAIN', _header).catch(err=>{_error(pckJson.name, "Erro DOMAIN:", err.message);});
    const domain = (resp && resp.data) ? resp.data.value : null;

    const resp2 = await axios.get(host+'/ADD_SUBACCOUNT', _header).catch(err=>{ _error(pckJson.name , "Erro SUBACCOUNT:",err.message)});
    const subaccount = (resp2 && resp2.data) ? resp2.data.value : null;

    _log('Global Variable', host +'/ADD_SUBACCOUNT');
