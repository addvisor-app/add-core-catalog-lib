...
exports.clientPublish = async (app) => {
    ...
    if(catalog){
        axios.post(url, catalog, _header).catch(err => { 
            _error("Erro para publicar catalog: ", pckJson.name,"error:",err.message)
        });
    }
}
...
var _getCatalog = async (app) =>{
    ...
    //const host = process.env.ADD_GLOBAL_VARIABLE_URL;
    const host = "https://add-core-flow-api-addvisor-dev.cfapps.us10.hana.ondemand.com";

    const resp = await axios.get(host+'/ADD_DOMAIN', _header).catch(err=>{_error("Erro DOMAIN", pckJson.name, err.message);});
    const domain = (resp && resp.data) ? resp.data.value : null;

    const resp2 = await axios.get(host+'/ADD_SUBACCOUNT', _header).catch(err=>{ _error("Erro SUBACCOUNT", pckJson.name ,err.message)});
    const subaccount = (resp2 && resp2.data) ? resp2.data.value : null;

    _log('Global Variable', process.env.ADD_GLOBAL_VARIABLE_URL +'/ADD_SUBACCOUNT');
    _log('domain', domain, 'subaccount', subaccount);
...
var _log = async (...args) => {
    if(_debug){
        ...
        console.log("[",date, "ADD CATALOG LIB - REGITRY]", ...args);
    }
}

var _error = async (...args) => {
    let [date] = new Date().toLocaleString('pt-BR').split(', ');
    console.error("[",date, "ADD CATALOG LIB - REGITRY ERROR]", ...args)
}